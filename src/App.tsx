...
const App = () => {
  ...
  const [perPage, setPerPage] = useState<number>(paramsPerPage && +paramsPerPage || 5);
  const [page, setPage] = useState<number>(paramsPage && +paramsPage || 1);
  ...
  const handleAmountPerPage = (e: any) => {
    setPerPage(+e.target.value);
    setPage(1);
  }

  const handleActivePage = (e: React.ChangeEvent<unknown>, numberPage: number) => {
    e.preventDefault();
    setPage(+numberPage)
  }

  const handlePrev = (e: React.ChangeEvent<unknown>) => {
    e.preventDefault();
    setPage(page - 1)
  }

  const handleNext = (e: React.ChangeEvent<unknown>) => {
    e.preventDefault();
    setPage(page + 1)
  }
